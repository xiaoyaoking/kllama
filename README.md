...
> **说明：** kllama 是 llama.cpp 的管理外壳，核心推理由 `llama-server` 完成。`pull` / `create` 不依赖 llama.cpp；**首次 `start` / `serve` / `setup` 会编译**。默认按显卡自动选择后端（仅核显 → CPU）。

## 编译 llama.cpp（`kllama setup`）

```bash
# 自动检测：独显 + CUDA/ROCm/Vulkan → 对应 GPU；仅核显或无工具链 → CPU
kllama setup

# 强制覆盖重编译
kllama setup --force --backend cuda
kllama setup --force --backend cpu
kllama setup --force --backend vulkan

# 额外 cmake 参数（可重复）
kllama setup --force --backend cuda --cmake-arg -DCMAKE_CUDA_ARCHITECTURES=89
```

| `--backend` | 说明 | cmake 开关 |
|-------------|------|------------|
| `auto` | 默认；检测独显与工具链 | 见下 |
| `cpu` | 纯 CPU（macOS 会关 Metal） | `-DGGML_METAL=OFF`（仅 macOS） |
| `cuda` | NVIDIA | `-DGGML_CUDA=ON` |
| `vulkan` | Vulkan（跨厂商） | `-DGGML_VULKAN=ON` |
| `metal` | Apple Silicon | `-DGGML_METAL=ON` |
| `hip` | AMD ROCm | `-DGGML_HIP=ON` |

`auto` 优先级：Apple Silicon→Metal → NVIDIA+CUDA→CUDA → AMD+ROCm→HIP → 独显+Vulkan→Vulkan → 仅核显/无工具链→CPU。构建信息写在 `~/.kllama/bin/build-info.json`。
...
| `kllama setup` | 编译安装 llama.cpp（可指定 backend；`start`/`serve` 也会自动触发） |
